...
def cv_random_flip(img_A, img_B, edge, label):
    # left right flip
...
def randomCrop_Mosaic(image_A, image_B, edge, label, crop_win_width, crop_win_height):
    image_width = image_A.size[0]
    image_height = image_A.size[1]
    random_region = (
        (image_width - crop_win_width) >> 1, (image_height - crop_win_height) >> 1, (image_width + crop_win_width) >> 1,
        (image_height + crop_win_height) >> 1)
    return image_A.crop(random_region), image_B.crop(random_region), edge.crop(random_region), label.crop(random_region)


def randomCrop(image_A, image_B, edge, label):
    border = 30
    image_width = image_A.size[0]
...
def randomRotation(image_A, image_B, edge, label):
    mode = Image.BICUBIC
    ...
    return image_A, image_B, edge, label
...
class ChangeDataset(data.Dataset):
    def __init__(self, root, trainsize, mosaic_ratio=0.75):
        ...
        self.edges = [self.edge_root + f for f in os.listdir(self.edge_root) if f.endswith('.jpg')
                    or f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        if p_ratio > self.mosaic_ratio:
            image_A, image_B, edge, gt = self.load_img_and_mask(index)
            image_A, image_B, edge, gt = cv_random_flip(image_A, image_B, edge, gt)
            image_A, image_B, edge, gt = randomCrop(image_A, image_B, edge, gt)
            image_A, image_B, edge, gt = randomRotation(image_A, image_B, edge, gt)
            image_A,image_B = colorEnhance(image_A,image_B)
            edge = randomPeper(edge)
            gt = randomPeper(gt)
        else:
            image_A, image_B, edge, gt = self.load_mosaic_img_and_mask(index)
            image_A, image_B, edge, gt = cv_random_flip(image_A, image_B, edge, gt)
            image_A, image_B, edge, gt = randomRotation(image_A, image_B, edge, gt)
            image_A,image_B = colorEnhance(image_A,image_B)
            edge = randomPeper(edge)
        ...
        return image_A, image_B, edge, gt


    def load_img_and_mask(self, index):
        A = Image.open(self.images_A[index]).convert('RGB')
        B = Image.open(self.images_B[index]).convert('RGB')
        mask1 = Image.open(self.edges[index]).convert('L')
        mask = Image.open(self.gts[index]).convert('L')
        return A, B, mask1, mask

    def load_mosaic_img_and_mask(self, index):
       indexes = [index] + [random.randint(0, self.size - 1) for _ in range(4)]
       img_a_a, img_a_b, mask1_a, mask_a = self.load_img_and_mask(indexes[0])
       img_b_a, img_b_b, mask1_b, mask_b = self.load_img_and_mask(indexes[1])
       img_c_a, img_c_b, mask1_c, mask_c = self.load_img_and_mask(indexes[2])
       img_d_a, img_d_b, mask1_d, mask_d = self.load_img_and_mask(indexes[3])
       ...
       croped_a_a, croped_a_b, mask1_crop_a, mask_crop_a = randomCrop_Mosaic(img_a_a.copy(), img_a_b.copy(), mask1_a.copy(), mask_a.copy(),crop_size_a[0], crop_size_a[1])
       croped_b_a, croped_b_b, mask1_crop_b, mask_crop_b = randomCrop_Mosaic(img_b_a.copy(), img_b_b.copy(), mask1_b.copy(), mask_b.copy(),crop_size_b[0], crop_size_b[1])
       croped_c_a, croped_c_b, mask1_crop_c, mask_crop_c = randomCrop_Mosaic(img_c_a.copy(), img_c_b.copy(), mask1_c.copy(), mask_c.copy(),crop_size_c[0], crop_size_c[1])
       croped_d_a, croped_d_b, mask1_crop_d, mask_crop_d = randomCrop_Mosaic(img_d_a.copy(), img_d_b.copy(), mask1_d.copy(), mask_d.copy(),crop_size_d[0], crop_size_d[1])

       croped_a_a, croped_a_b, mask1_crop_a, mask_crop_a = np.array(croped_a_a), np.array(croped_a_b), np.array(mask1_crop_a), np.array(mask_crop_a)
       croped_b_a, croped_b_b, mask1_crop_b, mask_crop_b = np.array(croped_b_a), np.array(croped_b_b), np.array(mask1_crop_b), np.array(mask_crop_b)
       croped_c_a, croped_c_b, mask1_crop_c, mask_crop_c = np.array(croped_c_a), np.array(croped_c_b), np.array(mask1_crop_c), np.array(mask_crop_c)
       croped_d_a, croped_d_b, mask1_crop_d, mask_crop_d = np.array(croped_d_a), np.array(croped_d_b), np.array(mask1_crop_d), np.array(mask_crop_d)
       ...
       top_mask1 = np.concatenate((mask1_crop_a, mask1_crop_b), axis=1)
       bottom_mask1 = np.concatenate((mask1_crop_c, mask1_crop_d), axis=1)
       mask1 = np.concatenate((top_mask1, bottom_mask1), axis=0)
       ...
       return img_a, img_b, mask1, mask

    def filter_files(self):
        assert len(self.images_A) == len(self.gts)
        assert len(self.images_A) == len(self.edges)
        assert len(self.images_A) == len(self.images_B)
        images_A = []
        images_B = []
        gts = []
        edges = []
        for img_A_path, img_B_path, edge_path, gt_path in zip(self.images_A, self.images_B, self.edges, self.gts):
...
class Test_ChangeDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image_A, image_B, edge, gt, file_name
